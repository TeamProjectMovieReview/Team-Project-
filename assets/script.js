...
const apiKey = '7b82484f';
const apiUrl = 'http://www.omdbapi.com/';

// Function to fetch movie data from OMDb API
async function fetchMovieData(movieTitle) {
    try {
        const response = await fetch(`${apiUrl}?t=${movieTitle}&apikey=${apiKey}`);
...
// Function to update HTML content with movie data
async function updateMovieInfo(movieTitle) {
    ...
    // Check if the API response includes a poster
    if (movieData.Poster && movieData.Poster !== 'N/A') {
        // Update poster image source
        posterElement.src = movieData.Poster;
    } else {
        // If no poster is available, display a placeholder image
        posterElement.src = 'placeholder.jpg';
    }
}

// Example usage: Call the function with a specific movie title
const movieTitle = 'The Shawshank Redemption';
updateMovieInfo(movieTitle);
